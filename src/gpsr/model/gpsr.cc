...
RoutingProtocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::gpsr::RoutingProtocol")
    ...
    .AddTraceSource ("HelloTx",
                     ...
                     MakeTraceSourceAccessor (&RoutingProtocol::m_hello),
                     "ns3::gpsr::RoutingProtocol::Hello")
  ;
  return tid;
}
...
float
RoutingProtocol::GetPdr ()
{
  float pdr = InPackets/OutPackets;
  InPackets = 0;
  OutPackets = 0;
  return pdr;
}
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  ...
  float n = float (rand()) / (float (RAND_MAX/GPSR_MAXJITTER) );
  JITTER  = Seconds(n);
...
RoutingProtocol::HelloTimerExpire ()
{
  ...
  float n = float (rand()) / (float (RAND_MAX/GPSR_MAXJITTER) );
  float n2 = -(float (rand()) / (float (RAND_MAX/GPSR_MAXJITTER) ));
 
  JITTER  = Seconds(n+n2);

  SendHello ();
  HelloIntervalTimer.Cancel ();
  HelloIntervalTimer.Schedule (HelloInterval + JITTER + m_delay);
}
...
RoutingProtocol::SendHello ()
{
  ...
  //uint64_t density = m_neighbors.GetSize();

  //if (density == 0) 
  //  probToSend = 1;
 // else
  //  probToSend = 1/double(density);
...
TypeHeader tHeader (GPSRTYPE_POS);
  PositionHeader hdr;
  p2->RemoveHeader (tHeader);
  
  if (tHeader.Get () == GPSRTYPE_POS)
    {
  if (m_ipv4->GetAddress (1, 0).GetLocal () == Ipv4Address ("10.0.0.51"))
        ++OutPackets;
    }
