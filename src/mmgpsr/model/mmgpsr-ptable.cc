#include "mmgpsr-ptable.h"
#include "ns3/simulator.h"
#include "ns3/log.h"
#include <algorithm>
#include <cmath>

NS_LOG_COMPONENT_DEFINE ("MMGpsrTable");
  ...
  for (i = m_table.begin (); !(i == m_table.end ()); i++)
    {
	  ...
	  bestN = i->second.first;
        }
    }

  dBD = CalculateDistance(position,  bestN);
  dSB = CalculateDistance(nodePos,  bestN);
  dmax = dBD + lambda*dSB;

  bestFoundID = SelectedNeighbor(dmax, position);
...
Ipv4Address 
PositionTable::SelectedNeighbor (double m_dmax, Vector position)
{
    ...
    for (i = m_table.begin (); !(i == m_table.end ()); i++)
    {
      if (CalculateDistance (i->second.first, position) <= m_dmax)
        {
          T_cum = m_Ttable.Get(i->first);
          if (T_cum.GetSeconds() > bestFoundT)
          {
	     bestFoundID = i->first;
	     bestFoundT = T_cum.GetSeconds();
