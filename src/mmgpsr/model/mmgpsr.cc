...
#define MMGPSR_LS_GOD 0

#define MMGPSR_LS_RLS 1

NS_LOG_COMPONENT_DEFINE ("MMGpsrRoutingProtocol");
...
struct DeferredRouteOutputTag : public Tag
{
  ...
  {
    static TypeId tid = TypeId ("ns3::mmgpsr::DeferredRouteOutputTag").SetParent<Tag> ();
    return tid;
  }
...
#define MMGPSR_MAXJITTER          (HelloInterval.GetSeconds () / 2)
/// Random number between [(-MMGPSR_MAXJITTER)-MMGPSR_MAXJITTER] used to jitter HELLO packet transmission.

//#define JITTER (Seconds (x->GetValue (-MMGPSR_MAXJITTER, MMGPSR_MAXJITTER))) 
//#define FIRST_JITTER (Seconds (x->GetValue (0, MMGPSR_MAXJITTER))) //first Hello can not be in the past, used only on SetIpv4



NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for MMGPSR control traffic, not defined by IANA yet
const uint32_t RoutingProtocol::MMGPSR_PORT = 666;
...
{
  static TypeId tid = TypeId ("ns3::mmgpsr::RoutingProtocol")
    ...
    .AddAttribute ("LocationServiceName", "Indicates wich Location Service is enabled",
                   EnumValue (MMGPSR_LS_GOD),
                   MakeEnumAccessor (&RoutingProtocol::LocationServiceName),
                   MakeEnumChecker (MMGPSR_LS_GOD, "GOD",
                                    MMGPSR_LS_RLS, "RLS"))
  ;
  return tid;
}
...
NS_LOG_FUNCTION (this << p->GetUid () << header.GetDestination () << idev->GetAddress ());
  if (m_socketAddresses.empty ())
    {
      NS_LOG_LOGIC ("No mmgpsr interfaces");
      return false;
    }
  NS_ASSERT (m_ipv4 != 0);
  NS_ASSERT (p != 0);
  ...
  if (m_ipv4->IsDestinationAddress (dst, iif))
    {
      ...
      TypeHeader tHeader (MMGPSRTYPE_POS);
      packet->RemoveHeader (tHeader);
      if (!tHeader.IsValid ())
        {
          NS_LOG_DEBUG ("MMGPSR message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
          return false;
        }
      
      if (tHeader.Get () == MMGPSRTYPE_POS)
        {
          PositionHeader phdr;
        ...
        while(m_queue.Dequeue (dst, queueEntry))
          {
            ...
            if (!tHeader.IsValid ())
              {
                NS_LOG_DEBUG ("MMGPSR message " << p->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
                return false;     // drop
              }
            if (tHeader.Get () == MMGPSRTYPE_POS)
              {
...
//std::cout<<"Entered in recovery Mode\n";
  ...
  Ipv4Address nextHop = m_neighbors.BestAngle (Position, myPos);
...
RoutingProtocol::NotifyInterfaceUp (uint32_t interface)
{
  ...
  if (l3->GetNAddresses (interface) > 1)
    {
      NS_LOG_WARN ("MMGPSR does not work with more then one address per each interface.");
    }
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvMMGPSR, this));
  socket->Bind (InetSocketAddress (Ipv4Address::GetAny (), MMGPSR_PORT));
...
void
RoutingProtocol::RecvMMGPSR (Ptr<Socket> socket)
{
  NS_LOG_FUNCTION (this << socket);
  Address sourceAddress;
  Ptr<Packet> packet = socket->RecvFrom (sourceAddress);

  TypeHeader tHeader (MMGPSRTYPE_HELLO);
  packet->RemoveHeader (tHeader);
  if (!tHeader.IsValid ())
    {
      NS_LOG_DEBUG ("MMGPSR message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
...
void RoutingProtocol::NotifyAddAddress (uint32_t interface, Ipv4InterfaceAddress address)
{
  ...
  if (l3->GetNAddresses ((interface) == 1))
    ...
    {
      NS_LOG_LOGIC ("MMGPSR does not work with more then one address per each interface. Ignore added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  ...
  if (socket)
    ...
    {
      NS_LOG_LOGIC ("Remove address not participating in MMGPSR operation");
    }
}
...
RoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  ...
  float n = float (rand()) / (float (RAND_MAX/MMGPSR_MAXJITTER) );
  JITTER  = Seconds(n);
...
RoutingProtocol::HelloTimerExpire ()
{
  ...
  float n = float (rand()) / (float (RAND_MAX/MMGPSR_MAXJITTER) );
  float n2 = -(float (rand()) / (float (RAND_MAX/MMGPSR_MAXJITTER) ));
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      if (iface.GetMask () == Ipv4Mask::GetOnes ())
        ...
        socket->SendTo (packet, 0, InetSocketAddress (destination, MMGPSR_PORT));
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif)
{
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid MMGPSR source address not found");
...
/*
  if (inRec == 1){
        ...
        m_neighbors.ChangeTrustStatus (previousHopIp, );
  }
*/
  if(inRec == 1 && CalculateDistance (myPos, Position) < CalculateDistance (RecPosition, Position)){
    ...
    hdr.SetInRec(0);
/*
    std::cout<<"No longer in Recovery\n";
    std::cout<<"myPos"<<myPos<<"\n";
    std::cout<<"Position"<<Position<<"\n";
    std::cout<<"RecPosition"<<RecPosition<<"\n";
    std::cout<<"LastPos"<<previousHop<<"\n";
*/
