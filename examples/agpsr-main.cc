...
GpsrExample::writeToFile(uint32_t lostPackets, uint32_t totalTx, uint32_t totalRx, double hopCount,double count, double delay){
  ...
  struct stat buf;
  std::string outputfile = "results_journal/pairs"+std::to_string(nPairs)+"/"+algorithm+std::to_string(size)+"_results.txt";
//std::string outputfile = "result/teste.txt";
...
GpsrExample::CreateNodes ()
{
 ...
 std::cout << "Creating  " << (unsigned)size << " nodes .."<<" with "<<nPairs<<" pairs..." <<"\n";
  ...
  m_traceFile = "/home/workfolder/ns-3.23/results_journal/tclFiles/speed/"+std::to_string(speed)+"/newNs2mobility"+std::to_string(size)+".tcl";
